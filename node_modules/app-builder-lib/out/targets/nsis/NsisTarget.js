...
class NsisTarget extends _core().Target {
  ...
  async buildInstaller() {
    ...
    const defines_uninstaller = Object.assign({}, defines);
    const commands_uninstaller = Object.assign({}, commands);
    ...
    const script = isPortable ? await (0, _fsExtra().readFile)(path.join(_nsisUtil().nsisTemplatesDir, "portable.nsi"), "utf8") : await this.computeScriptAndSignUninstaller(defines_uninstaller, commands_uninstaller, installerPath, sharedHeader); // copy outfile name into main options, as the computeScriptAndSignUninstaller function was kind enough to add important data to temporary defines.
